...
# CHANGE: A trailhead's score is the number of unique hiking trails from that trailhead.

# Example: This has 9 trailheads with scores of 20, 24, 10, 4, 1, 4, 5, 8, and 5
...
class Topomap
  ...
  attr_reader :check_around, :get_trailhead_score, :total_trailhead_score
  ...
  def check_around(line_num, char_pos, value)
    ...
    # puts "  num unique high points for this trailhead: "+@high_points.length.to_s
  ...
  def get_trailhead_score
    ...
    for row, row_id in @topo_map.map.with_index
      for char, char_pos in row.map.with_index
        height=char.to_i
        if height==0 # this may be a trailhead - check it and get the number of trails from it
