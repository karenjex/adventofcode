# Goal: Find number of characters that must be processed before the first start-of-packet marker is detected

# A start-of-packet marker is a sequence of four characters that are all different in the datastream
...
current_buffer=[]          # track latest 4-characters
...
with open(inputfile) as f:
    while 1:
        ...
        if len(current_buffer)<4:
            ...
            matched=False
            for i in range(0,4):                # 2 loops to compare each char in current_buffer against the others to find any matches
